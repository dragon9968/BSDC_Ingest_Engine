...
class MappingValidator:
    ...
    def __init__(self, mapping_file_path: str = None):
        if mapping_file_path:
            ...
            # 🎯 Trỏ thẳng vào thư mục INGEST chứa raw data từ SharePoint kéo về
            ingest_dir = Path(settings.LOCAL_INGEST_DIR)
            
            # Quét lấy file Excel gốc (.xlsx) có chứa từ "mapping" trong tên
            mapping_files = [f for f in ingest_dir.glob("*.xlsx") if "mapping" in f.name.lower()]
            
            if not mapping_files:
                raise FileNotFoundError(
                    f"❌ Không tìm thấy file Excel Mapping gốc (.xlsx) nào trong thư mục ingest: {ingest_dir}\n"
                    f"💡 Đảm bảo tên file trên SharePoint có chứa từ 'Mapping' (Ví dụ: MediCoop Data Mapping.xlsx)"
                )
    ...
    def validate(self) -> tuple[bool, dict]:
        ...
        try:
            ...
            return False, {"System": [f"❌ Không thể mở file Excel Mapping: {e}"]}
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
                ...
                # 1. Nhận diện dòng Header tiêu chuẩn
                ...
                if col_idx_map:
                    ...
                    # Check Lỗi 1: Khai báo Column mà thiếu Data File
                    if column_val and not effective_data_file:
                        sheet_errors.append(
                            f"Dòng {row_idx:04d} | Field '{field_val}': Có điền Column='{column_val}' nhưng chưa khai báo Data File ở dòng này hay phía trên!"
                        )

                    # Check Lỗi 2: Converted = Yes nhưng trống trơn
                    if is_converted and not has_source_mapping and not has_notes:
                        sheet_errors.append(
                            f"Dòng {row_idx:04d} | Field '{field_val}': Đánh dấu Converted='Yes' nhưng KHÔNG CÓ Mapping (Data File/Column) lẫn Notes!"
                        )

            # Check Lỗi 3: Sheet bị để trống hoàn toàn (Bắt buộc ghi N/A ở cột Notes nếu bỏ qua)
            if has_header and mapped_field_count == 0:
                sheet_errors.append(
                    f"Toàn bộ Sheet này đang bị bỏ trống (Chưa map bất kỳ Field nào). "
                    f"💡 Hướng xử lý: Nếu Credit Union KHÔNG DÙNG module này, hãy gõ chữ 'N/A' vào cột Notes của 1 dòng bất kỳ để xác nhận!"
                )
